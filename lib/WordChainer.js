...
class WordChainer {
  ...
  async getNextWord(text) {
    ...
    if (words !== null) {
      ...
      const candidateWords = words.filter(value => Math.ceil(value[1]) === scoreMax);
      if (candidateWords && candidateWords.length >= 1) {
        return candidateWords[Math.floor(Math.random() * candidateWords.length)][0];
      }
    }
    return null;
  }
